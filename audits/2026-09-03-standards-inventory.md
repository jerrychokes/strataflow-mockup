# How standards are governed today — an inventory

**Measured 3 September 2026. This is evidence, not scope.**

## Status: the directive that prompted this was withdrawn

A directive — *"make all standards configurable across the platform"* — was given
on 3 September 2026 and **withdrawn the same day**. Nothing in the wave programme
depends on it, no wave was re-sequenced for it, and it places no requirement on
any later work. It is recorded here only so that a reader who finds this file in
the history knows why the measurement was taken.

**What survives is the measurement**, because it is worth keeping on its own
terms: it is the first count of how many standards this catalogue holds and how
well governed each one is, and several of its findings are defects that stand
whether or not anything is ever made configurable.

Three of those are worth reading even if nothing else here is:

- The **nitrate holding time** — the rule that removes a result from evaluation
  entirely — appears six times in four different spellings, and no register,
  screen or route owns holding times anywhere in 75 screens.
- The **analyte dictionary's reporting limits are hard-coded HTML table rows** in
  `screens.mjs`, and those numbers decide censoring, the indeterminate verdict on
  seven cadmium results, which branch of the 5 × LOR rule applies, and the width
  of a censored interval on a statutory return. It is the most consequential
  unversioned artefact in the catalogue.
- **The TARP is stricter than the licence** — Level 2 escalates on two consecutive
  rounds where condition 12(c) needs three — and it carries no version, no
  effective span, no source and no approver.

Both of the first two were independently re-measured before being recorded, and
both held.

Anything in the sections below that reads as a plan — what a register would carry,
what would break if one were built — is the analysis as it was written while the
directive stood. **It is not a commitment and nothing is scheduled against it.**
...
## What a naive versioning model would break (analysis only — nothing is planned)
