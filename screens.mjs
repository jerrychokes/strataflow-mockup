  ...
  return (
    ...
    cols(
      ...
      panel(
        ...
        `<p class="mk-tight">${I.checks.withoutInstrument.length} of the monitored bores and instrumented points have never had an instrument in them: ${I.checks.withoutInstrument.map((c) => loc(c)).join(', ')}. That is a fact about the network rather than a hole in this register, and it is on the screen because a register that lists only what it holds cannot be used to ask what is missing.</p>` +
  ...
  return (
    ...
    cols(
      panel(
        ...
        `<p class="mk-tight">The correction re-derived every water level in its reach — including these ${raised[0].levels}. The spike is still ${raised[0].swing.toFixed(3)} m and still there, because <strong>a compensation rule is not a filter</strong>. Removing what it cannot explain would destroy the evidence for the explanation, and the thing that most needs explaining is the thing most likely to be removed. One coincidence to read past: the spike's ${raised[0].swing.toFixed(3)} m and the correction's ${L.correction.shift.toFixed(2)} m shift are near-identical magnitudes with no shared cause — one is water moving at the screen over five hours, the other a sensor re-seated deeper and applied as a constant to every level in the reach. Neither is an artefact of the other.</p>` +
    ...
    cols(
      ...
      panel(
        ...
        `<p class="mk-tight"><strong>A censored value is the laboratory’s own notation.</strong> <span class="sf-result sf-result--censored"><span class="sf-result__value">&lt; ${esc(PFAS_LIMITS.lor.toFixed(1))}</span></span> asserts that ${esc(R.subcontract.laboratory)} looked for the analyte and reported it below the limit of reporting. There is no version of these ${R.cellsWithdrawn} cells that is <em>nearly</em> right, because the assertion is about an act that did not happen.</p>` +
          `<p class="mk-tight">The alternative reading — that the analyses happened and the manifest, the chain and the receipt all under-record them — would have to be drawn as three more bottles invented to justify a summary row — five in all against the 38 the receipt reconciled — plus a second consignment and a second receipt that nobody signed. That is a fabricated chain of custody in a system of record, and it is a worse defect than the one being fixed. It is the same trade <a class="mk-ref" href="#hydrograph">the trigger line</a> was refused on the day before.</p>` +
